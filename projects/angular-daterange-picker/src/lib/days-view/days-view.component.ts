import { Component, Input, OnDestroy, Output,
  EventEmitter, ViewChildren, ElementRef, QueryList, AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
export class DaysViewComponent implements OnDestroy, AfterViewInit {
  ...
  @Input() locale: string;
  @Output() daySelected = new EventEmitter<IDateSelectedEvent>();

  @ViewChildren('dayElement', { read: ElementRef }) dayEls: QueryList<ElementRef>;
  ...
  constructor(public picker: DatepickerService, public dayService: DaysService, private cdr: ChangeDetectorRef) {
  ...
  public selectDay(day: number): void {
    this.dayService.setDates(day, this.date);
  }
  ...
  private setHeaderTitle(): void {
    ...
    this.picker.headerLeft = this.date.toLocaleDateString(this.locale, options);
  }
  ...
  ngAfterViewInit(): void {
    this.dayService.initElements(this.dayEls);
    this.dayService.onDateSelected.subscribe((val) => {
      this.daySelected.emit(val);
    });
  }
